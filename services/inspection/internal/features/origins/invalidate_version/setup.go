...
func Setup(d Dependencies) error {
	if d.DB == nil || d.Bus == nil || d.Service.DB == nil {
		return fmt.Errorf("slice origins/invalidate_version: missing dependency")
	}
	return d.Bus.RegisterCommand(Command{}, func(ctx context.Context, raw any) (any, error) {
		...
		if err := (tenanttx.Runner{DB: d.DB}).Within(ctx, command.TenantID, func(tx *gorm.DB) error {
			if err := tx.Where("tenant_id=? AND id=?", command.TenantID, command.VersionID).First(&version).Error; err != nil {
				return err
			}
			if err := tx.Where("tenant_id=? AND id=?", command.TenantID, version.OriginID).First(&origin).Error; err != nil {
				return err
			}
