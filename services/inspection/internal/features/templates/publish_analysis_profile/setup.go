package publish_analysis_profile
...
func Setup(d Dependencies) error {
	if d.DB == nil || d.Bus == nil {
		return fmt.Errorf("slice templates/publish_analysis_profile: missing dependency")
	}
	...
	return d.Bus.RegisterCommand(Command{}, func(ctx context.Context, raw any) (any, error) {
		c := raw.(Command)
		return s.PublishProfile(ctx, c.TenantID, c.Key, c.IdempotencyKey, c.DefinitionJSON)
	})
}
