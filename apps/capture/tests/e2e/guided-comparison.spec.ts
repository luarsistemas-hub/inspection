...
test("guided comparison shows each real reference, previews photos, and advances after upload", async ({ page, browserName }) => {
  ...
  await page.route("**/*graphql*", (route) => {
    ...
    if (query.includes("externalCapture")) return route.fulfill({ json: { data: { externalCapture: bootstrap } } });
    ...
    if (query.includes("saveCaptureMetadata")) return route.fulfill({ json: { data: { saveCaptureMetadata: { media: { id: `media-${createdUploads}`, status: "READY" }, userErrors: [], clientMutationId: "c" } } } });
  ...
  await page.getByLabel("Tirar foto").setInputFiles({ name: "cozinha.png", mimeType: "image/png", buffer: photo });
