...
const storageOrigin = (() => { try { return new URL(process.env.NEXT_PUBLIC_STORAGE_URL ?? "http://localhost:9002").origin; } catch { return "http://localhost:9002"; } })();
const nextConfig: NextConfig = {
  ...
  async headers() { return [{ source: "/(.*)", headers: [
    ...
    { key: "Content-Security-Policy", value: `default-src 'self'; connect-src 'self' ${apiOrigin} ${oidcOrigin}; img-src 'self' ${storageOrigin} data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; frame-ancestors 'none'` }
  ] }]; }
};
