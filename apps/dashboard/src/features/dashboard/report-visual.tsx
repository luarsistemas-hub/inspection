...
import { presentChangeType, presentClassification, presentComparisonStatus, presentCoverageStatus } from "./presentation";
...
function ReportEvidence({ requirements, evidence, findings, onMediaError }: { requirements: Array<{ key: string; section: string; label: string; instructions?: string | null; coverageStatus?: string | null; comparisonStatus?: string | null }>; evidence: Array<{ id: string; requirementKey: string; role: string; description?: string | null; capturedAt?: string | null; flags: string[]; availability: string; url?: string | null }>; findings: Array<{ category?: string; changeType?: string | null; title: string; description: string; severity: string; recommendedAction: string; evidenceIds: string[] }>; onMediaError?: () => void }) {
  ...
  return <section className="report-requirements" aria-label="Evidências do laudo">
    {requirements.map((requirement) => {
      ...
      return <section className="report-requirement" key={requirement.key}><header><span>{requirement.section}</span><h3>{requirement.label}</h3>{requirement.instructions && <p>{requirement.instructions}</p>}{requirement.coverageStatus && <small>{presentCoverageStatus(requirement.coverageStatus)}{requirement.comparisonStatus && ` · ${presentComparisonStatus(requirement.comparisonStatus)}`}</small>}</header><div className="report-comparison">{reference.length > 0 && <EvidenceColumn title="Referência" items={reference} onMediaError={onMediaError} />}<EvidenceColumn title={reference.length > 0 ? "Vistoria atual" : "Fotos da vistoria"} items={current} onMediaError={onMediaError} /></div>{related.length > 0 ? <section className="report-findings" aria-label={`Constatações de ${requirement.label}`}>{related.map((finding) => <article key={`${finding.title}-${finding.description}`}><strong>{finding.title} · {finding.severity}{finding.changeType && ` · ${presentChangeType(finding.changeType)}`}</strong><p>{finding.description}</p><span>Ação recomendada: {finding.recommendedAction}</span></article>)}</section> : requirement.comparisonStatus === "NOT_APPLICABLE" ? <p className="report-empty">Sem achados relevantes</p> : null}</section>;
    })}
