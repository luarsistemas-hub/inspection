...
test("completes the real onboarding flow and creates the first inspection", async ({ page }, testInfo) => {
  ...
  await onboarding.expectStep("Agency");
  await onboarding.saveAgency(`Imobiliária E2E ${suffix}`);
  await onboarding.expectStep("Property");
  ...
  await onboarding.expectStep("Reference photos");
  await onboarding.saveChecklistOrigin();
  await onboarding.expectStep("Participant");
