...
sed \
  ...
  -e 's/^INSPECTION_WEB_PORT=.*/INSPECTION_WEB_PORT=3100/' \
  "$workspace/.env.example" >"$tmp_dir/env"
printf '%s\n' 'NEXT_PUBLIC_INSPECTION_API_URL=http://localhost:8080/graphql' >>"$tmp_dir/env"

actual="$(PATH="$tmp_dir/bin:$PATH" INSPECTION_ENV_FILE="$tmp_dir/env" "$workspace/scripts/dev.sh" web)"
...
[[ "$actual" == *"$expected_origin"* ]] || {
  printf 'expected allowed origin %s, got %s\n' "$expected_origin" "$actual" >&2
  exit 1
}
